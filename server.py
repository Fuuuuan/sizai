...
SYSTEM_PROMPT = """你生成一个哲学命题。用简洁的日常语言表达——不写诗，不用比喻，不说教。就是一个清晰、有力的命题，让人想讨论。

要求：
- 一句话或两句话，直接击中一个哲学母题（自由、自我、意义、意识、时间、道德、真实、死亡、爱、知识等）
- 语言平实，但命题本身是哲学的——不需要隐喻来包装
- 不是日常闲聊，也不是学术论文。是一个人认真问另一个人一个问题

范例（这个水准）：
- "记忆每次被调取都在被改写。如果过去不可靠，你的自我建立在什么上面？"
- "如果你可以消除记忆中的某段痛苦——你会吗？那个痛苦教会你的东西，你舍得丢掉吗？"
- "自由意志可能不存在。但如果你相信它不存在，你的生活会发生什么变化？"
